...
export const Gradient = memo((props: GradientProps) => {
    ...
    const { status, send } = useCommandFeedback({ sourceIdx, onSuccess: clearPendingColors, deviceFriendlyName: device.friendly_name });
    ...
    return (
        <>
            ...
            <div className="flex flex-row gap-2 items-center">
                <ApplyButton status={status} hasLocalChanges={hasLocalChanges} onClick={onGradientApply} minimal={minimal} />
            </div>
        </>
    );
});
